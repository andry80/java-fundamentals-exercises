...
//        throw new ExerciseNotCompletedException();
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream().collect(Collectors.toMap(s -> s.getEmail().split("@")[1],
                s -> accounts.stream().filter(s1 -> s1.getEmail().split("@")[1].equals(s.getEmail().split("@")[1])).collect(Collectors.toList()),
...
//        throw new ExerciseNotCompletedException();
    ...
    public BigDecimal getBalanceByEmail(String email) {
        return accounts.stream()
                ...
                .map(a -> a.getBalance())
                .findFirst()
                .orElseThrow(()->{throw new EntityNotFoundException("Cannot find Account by email="+email);});
...
//        throw new ExerciseNotCompletedException();
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream()
                .map(accounts -> accounts.getFirstName().split(""))
                .flatMap(Arrays::stream)
                .map(a -> a.charAt(0))
                .collect(Collectors.toMap(chr -> (Character) chr, lng -> (Long) 1l, (a, b) -> a + b));
//        throw new ExerciseNotCompletedException();
    }
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames(int nameLengthBound) {
        return accounts.stream()
                .map(accounts -> new String[] {accounts.getFirstName(), accounts.getLastName()})
                .flatMap(Arrays::stream)
                .filter(a -> a.length() >= nameLengthBound)
                .map(a -> a.split(""))
                .flatMap(Arrays::stream)
